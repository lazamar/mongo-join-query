...
const toPath = fieldList => fieldList.map(f => f.name).join(".");

// getLookups Returns an Either because we cannot perform the aggregation if there
// is an array in any field other than the very first in the fieldList.
// [Object]
const getLookups = fieldList =>
    fieldList
        // index refers to the field index in fieldList
        .map((field, index) => ({ field, index }))
        .filter(({ field }) => isLookupField(field))
        .map(({ field, index }) => {
            // Path from root up to the current field
            const fieldsTillHere = fieldList.slice(0, index + 1);
            const fieldPath = toPath(fieldsTillHere);
            return [
                {
                    $lookup: {
...
// Given a poppulation tree, returns the node that can be grouped.
// Returns Left if there is a deep array being populated
// Node -> Either String Node
const getGroupingNode = root =>
    trees.reduce(
        (acc, node) =>
            acc.chain(() => {
...
const generateGrouping = (Model, root) =>
    getGroupingNode(root)
        .map(trees.getContent)
        .map(field => {
            const tempField = `_temp_${field.name}`;
                ...
                : Maybe.of({
                    unwind: [
                        ...
                        },
                        {
                             ...
                             */
                            $addFields: { [tempField]: `$${field.name}` }
                        }
                    ],
                    group: [
                        {
                        ...
                        { $group: groupingObject(Model, field.name) }
                    ]
                });
        });
...
//             traverse(Either.of, v => generateGrouping(Model, v), populationTrees)
//                 .map(groupings =>
//                     groupings
//                         .filter(v => v.isJust)
//                         .map(v => v.value)
//                         .reduce(
//                             (groups, g) => ({
//                                 unwind: [...groups.unwind, ...g.unwind],
//                                 group: [...groups.group, ...g.group]
//                             }),
//                             { unwind: [], group: [] }
//                         )
//                 )
//                 .map(groupings => ({ groupings, populationTrees }))
//         )
//         .bimap(err => `Invalid population path: ${err}`, v => v)
//         .map(({ groupings, populationTrees }) => {
//             const lookups = populationTrees
//                 .map(treeToLookup)
//                 .reduce((acc, v) => [...acc, ...v], []);
//
//             return [...groupings.unwind, ...lookups, ...groupings.group];
//         });
...
const treeToPipeline = (Model, populationTree) =>
    generateGrouping(Model, populationTree).map(mGrouping => {
        ...
        return [].concat(grouping.unwind, lookup, grouping.group);
    });
...
const populationPipeline = (Model, populate) =>
    createPopulationTrees(Model, populate)
        ...
        .bimap(err => `Invalid population path: ${err}`, v => v)
        .map(pipelines => [].concat(...pipelines)); // flatten array of arrays
